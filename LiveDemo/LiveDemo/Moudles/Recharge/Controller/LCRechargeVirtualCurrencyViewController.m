//
//  LCRechargeVirtualCurrencyViewController.m
//  LiveDemo
//
...
@interface LCRechargeVirtualCurrencyViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic , weak) UILabel *zhzsLabel;

@end

@implementation LCRechargeVirtualCurrencyViewController
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 1){
        //联系人 银行卡  虚拟币
        return [tableView fd_heightForCellWithIdentifier:@"LCVirtualCurrencyTableViewCell" configuration:^(LCVirtualCurrencyTableViewCell *cell) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    tipLabel.text = section == 0? KLanguage(@"请输入充值数量"):KLanguage(@"请转账到以下帐号：转账后请提交信息");
...
//        make.width.lessThanOrEqualTo(kUI_Width(250));
    }];
    if(section == 0){
        UILabel *zhzsLabel = [[UILabel alloc]initWithFrame:CGRectZero];

        zhzsLabel.textColor = Color(@"#333333");
        zhzsLabel.font = BoldFont(16);
        [view addSubview:zhzsLabel];
        self.zhzsLabel = zhzsLabel;
        [zhzsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(tipLabel.mas_right).offset(kUI_Width(4));
            make.height.equalTo(kUI_Width(16));
            make.top.equalTo(0);
            make.right.equalTo(-kUI_Width(16));
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 1){
        LCVirtualCurrencyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LCVirtualCurrencyTableViewCell"];
        ...
        cell.submitBlock = ^{
            if(!weakSelf.viewModel.selectMoneyModel){
                [SVProgressHUD showNoMaskViewWithInfo:KLanguage(@"请输入充值数量") ];
                return;
            }
            [SVProgressHUD showWithMaskView];
            [weakSelf.viewModel.bankOrVirtualSubmitCommand execute:@(YES)];
        };
//        cell.submitBlock = ^(NSString * _Nonnull countStr) {
//
//        };
        return cell;
    }
...
//    cell.dataArr = self.viewModel.moneyArray;
    ...
    cell.inputBlock = ^(NSString * _Nonnull input) {
        if(input.length ){
            weakSelf.zhzsLabel.text = [NSString stringWithFormat:@"%@*%@=%.2f%@",input,weakSelf.viewModel.dataModel.usdtretio,input.integerValue*1.0 *weakSelf.viewModel.dataModel.usdtretio.integerValue,KLanguage(@"钻石")];
        }
...
- (LCBaseTableView *)mainTableView{
    if (_mainTableView == nil){
        ...
        [_mainTableView registerClass:[LCVirtualCurrencyTableViewCell class] forCellReuseIdentifier:@"LCVirtualCurrencyTableViewCell"];
