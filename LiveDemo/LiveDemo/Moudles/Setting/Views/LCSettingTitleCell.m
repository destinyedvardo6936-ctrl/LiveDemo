...
@end
@implementation LCSettingTitleCell
...
- (void)setDataModel:(LCSettingModel *)dataModel{
    ...
    [[RACObserve(_dataModel, needAccess) takeUntil:self.rac_prepareForReuseSignal] subscribeNext:^(NSNumber *  _Nullable x) {
        @strongify(self)
        self.rightImgView.hidden = ![x boolValue];
    }];
    RAC(self.mainTitleLabel,text) = [RACObserve(_dataModel, rightValue) takeUntil:self.rac_prepareForReuseSignal];
}
#pragma mark---- 懒加载 ----
- (UIView *)backView{
    if(_backView == nil){
