...
@property(nonatomic,strong)BuyVip *buyVip;
@end
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (_controlView.playBtn.hidden == NO) {
        NSDictionary *dic = _currentVideoDic;
        if([dic.allKeys containsObject:@"isvip"] && [dic[@"isvip"] boolValue]){
            //需要vip
          
            if([LCUserInfoManager shareManager].userInfo.vip_type.intValue == 0){
                if(!_buyVip.superview){
                    _buyVip = [[BuyVip alloc] init];
                    [self.view.window addSubview:_buyVip];
                    [_buyVip.btn1 addTarget:self action:@selector(clickNext) forControlEvents:UIControlEventTouchUpInside];
                    [_buyVip.btn3 addTarget:self action:@selector(clickVIP) forControlEvents:UIControlEventTouchUpInside];
                    [_controlView pauseVideo];
...
// player的tag值必须在cell里设置
    ...
    self.player.playerPrepareToPlay = ^(id<ZFPlayerMediaPlayback>  _Nonnull asset, NSURL * _Nonnull assetURL) {
        ...
        [self videoStart];
        NSInteger index = [_videoUrls indexOfObject:assetURL];
        NSDictionary *dic = [_videoList objectAtIndex:index];
        if([dic.allKeys containsObject:@"isvip"] && [dic[@"isvip"] boolValue]){
            //需要vip
...
//            [self addLookTimeForToday:[NSString stringWithFormat:@"%.0f",self.player.totalTime]];
    ...
    self.player.zf_playerAppearingInScrollView = ^(NSIndexPath * _Nonnull indexPath, CGFloat playerApperaPercent) {
...
-(void)contorEvent:(NSString *)eventStr andGes:(ZFPlayerGestureControl*)gesControl{
    if ([eventStr isEqual:@"控制-单击"]) {
        ...
        if (_videomore && _videomore.top < SCREEN_HEIGHT) {
            [self hideMoreView];

        }else if (_textField.isFirstResponder ){
            [_textField resignFirstResponder];
            
        }else if (_emojiV.top == SCREEN_HEIGHT - (EmojiHeight+KSafeHeight))
        { 
            [UIView animateWithDuration:0.1 animations:^{
                _toolBar.frame = CGRectMake(0, SCREEN_HEIGHT - 50-kStatusBarHeight, SCREEN_WIDTH, 50+kStatusBarHeight);
                //_toolBar.frame = CGRectMake(0, SCREEN_HEIGHT + 10, SCREEN_WIDTH, 50);
...
-(void)showtextfield{
    if (!_toolBar) {
        ...
        _finishbtn.frame = CGRectMake(SCREEN_WIDTH - 44,8,34,34);
...
-(void)atFrends {
    ...
    if (!_finishbtn.selected) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _toolBar.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
...
- (UIButton *)goBackBtn{
    if (!_goBackBtn) {
        ...
        _goBackBtn.frame = CGRectMake(10, 22+kStatusBarHeight, 40, 40);
        ...
        _goBackShadow.frame = CGRectMake(0, 0, 64, 64+kStatusBarHeight);
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (!decelerate) {
        if (self.player.currentPlayIndex == 0 ) {
            [SVProgressHUD showMaskViewWithFailure:KLanguage(@"已经到顶了哦")];
      
        }
        if (self.player.currentPlayIndex+1 == _videoList.count) {
            [SVProgressHUD showMaskViewWithFailure:KLanguage(@"没有更多视频")];
        }
    }
}
...
-(void)cellBlockBack:(NSString *)eventType andInfo:(NSDictionary *)eventDic andIndex:(NSIndexPath *)indexPath{
    if ([eventType isEqual:@"评论"]) {
        ...
        [self doenjoy];
    }else if ([eventType isEqual:@"游戏消失"]) {
        self.collectionView.scrollEnabled = YES;
        _toolBar.hidden = NO;
    }else if ([eventType isEqual:@"游戏出现"]) {
...
-(void)pullData{
    if (_isLoadingMore || [NSString checkNull:minstr(_sourceBaseUrl)]) {
        return;
    }
    ...
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:nil success:^(id  _Nullable result) {
        ...
        if ([_fromWhere isEqual:@"YBTakeSameVideoVC"]) {
            //注意拍摄同款的视频列表在info[0]里边的videolist
            NSDictionary *infoDic = [info firstObject];
            infoA = [NSArray arrayWithArray:[infoDic valueForKey:@"videolist"]];
        }
        if ([_fromWhere isEqual:@"topicDetailsVC"]) {
            //注意标签的视频列表在info[0]里边的list
            NSDictionary *infoDic = [info firstObject];
            infoA = [NSArray arrayWithArray:[infoDic valueForKey:@"list"]];
        }
        if (_pages==1) {
        ...
        if (_videoList.count<=0) {
            [SVProgressHUD showMaskViewWithFailure:KLanguage(@"暂无更多视频哦~")];
        }
...
-(void)startWatchVideo{
    NSString *url = @"Video.startWatchVideo";
...
-(void)endWatchVideo{
    NSString *url =@"Video.endWatchVideo";
...
-(void)videoStart {
    ...
    NSString *random_str = [[NSString stringWithFormat:@"%@-%@-#2hgfk85cm23mk58vncsark",[LCUserInfoManager shareManager].userInfo.ID,_videoID] md5String];
    NSString *url = [NSString stringWithFormat:@"Video.addView&uid=%@&token=%@&videoid=%@&random_str=%@",[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,_videoID,random_str];
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:nil success:^(id  _Nullable result) {
...
-(void)videoEnd {
    ...
    NSString *random_str = [[NSString stringWithFormat:@"%@-%@-#2hgfk85cm23mk58vncsark",[LCUserInfoManager shareManager].userInfo.ID,_videoID] md5String];
    NSString *url = [NSString stringWithFormat:@"Video.setConversion&uid=%@&token=%@&videoid=%@&random_str=%@",[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,_videoID,random_str];
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:nil success:^(id  _Nullable result) {
...
//    }
    NSString *url = [NSString stringWithFormat:@"Video.setComment&videoid=%@&content=%@&uid=%@&token=%@&touid=%@&commentid=%@&parentid=%@&at_info=%@",_videoID,path,[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,_hostID,@"0",@"0",at_json];
    ...
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:nil success:^(id  _Nullable result) {
        ...
        [SVProgressHUD showMaskViewWithFailure:minstr([result valueForKey:@"msg"])];
        weakSelf.textField.text = @"";
        weakSelf.textField.placeholder = KLanguage(@"说点什么...");
...
- (void)messgaebtnWithIndex:(NSIndexPath *)indexPath {
    ...
    WS(weakSelf);
    if (!_comment) {
        _comment = [[commentview alloc]initWithFrame:CGRectMake(0,SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT) hide:^(NSString *type) {
            [UIView animateWithDuration:0.3 animations:^{
                weakSelf.comment.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
...
//    [_comment getNewCount:[_comments intValue]];
    [UIView animateWithDuration:0.3 animations:^{
        weakSelf.comment.frame = CGRectMake(0,0,SCREEN_WIDTH, SCREEN_HEIGHT);
...
-(void)morebtn:(id)sender {
    if (!_videomore) {
        WS(weakSelf);
        NSArray *array = [LCConfigManager shareManager].configModel.share_type;
        CGFloat hh = SCREEN_HEIGHT/3+30+KSafeHeight;
        if (array.count == 0) {
            hh = SCREEN_HEIGHT/3/2+30+KSafeHeight;
        }
        _videomore = [[videoMoreView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT+20, SCREEN_WIDTH, hh) andHostDic:_currentVideoDic cancleblock:^(id array) {
...
-(void)showMoreView{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        weakSelf.videomore.frame = CGRectMake(0, SCREEN_HEIGHT - SCREEN_HEIGHT/3-50-KSafeHeight, SCREEN_WIDTH, SCREEN_HEIGHT/3+50+KSafeHeight);
        NSArray *array = [LCConfigManager shareManager].configModel.share_type;
        //如果没有分享
        if (array.count == 0) {
            weakSelf.videomore.frame = CGRectMake(0, SCREEN_HEIGHT - SCREEN_HEIGHT/3/2-50-KSafeHeight, SCREEN_WIDTH, SCREEN_HEIGHT/3/2+50+KSafeHeight);
        }
...
-(void)hideMoreView{
    ...
    WS(weakSelf);
    [UIView animateWithDuration:0.5 animations:^{
        weakSelf.videomore.frame = CGRectMake(0, SCREEN_HEIGHT +20, SCREEN_WIDTH, SCREEN_HEIGHT/2.2);
        NSArray *array = [LCConfigManager shareManager].configModel.share_type;
        if (array.count == 0) {
            weakSelf.videomore.frame = CGRectMake(0, SCREEN_HEIGHT +20, SCREEN_WIDTH, SCREEN_HEIGHT/2.2/2);
        }
