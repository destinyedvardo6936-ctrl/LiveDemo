...
-(instancetype)initWithDic:(NSDictionary *)subdic{
    ...
    if (self) {
        ...
        if ([_touid intValue] > 0) {
            _content = [NSString stringWithFormat:@"%@%@:%@",KLanguage(@"回复"),[_touserinfo valueForKey:@"user_nicename"],[subdic valueForKey:@"content"]];
        }else{
...
-(void)setmyframe:(detailmodel *)model{
    ...
    CGSize size = [reply1 boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 90, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil].size;
    _contentRect = CGRectMake(0,30, size.width, size.height);
...
//        CGSize size = [reply1 boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 120, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size;
...
//         CGSize size2 = [reply boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 120, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size;
//        _ReplyRect = CGRectMake(0, 20, size2.width+20, size2.height);
//        _rowH = MAX(0, CGRectGetMaxY(_ReplyRect)) + 15;
//    }
//    else{
//        CGSize size = [_content boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 120, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size;
