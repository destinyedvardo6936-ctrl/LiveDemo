...
-(void)reloaddata:(NSString *)from{
    ...
    count+=1;
    _textField.text = @"";
    _textField.placeholder = KLanguage(@"说点什么...");
    NSString *url = [NSString stringWithFormat:@"Video.getComments&videoid=%@&p=%d&uid=%@",_videoid,count,[LCUserInfoManager shareManager].userInfo.ID];
    ...
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:nil success:^(id  _Nullable result) {
        ...
        weakSelf.allCommentLabels.text = [NSString stringWithFormat:@"%d %@",allcomments,KLanguage(@"评论")];
...
//        _modelarray = [NSMutableArray array];
        ...
        _toolBar = [[UIView alloc]initWithFrame:CGRectMake(0,SCREEN_HEIGHT - 50-KSafeHeight, SCREEN_WIDTH, 50+KSafeHeight)];
        _toolBar.backgroundColor = ColorAlpha(@"#f4f5f6", 1);//RGB(248, 248, 248);
        
        //_toolBar顶部横线 和 顶部 view分割开
        UILabel *lineso = [[UILabel alloc]initWithFrame:CGRectMake(0,0,SCREEN_WIDTH,1)];
        lineso.backgroundColor = [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1];//[UIColor groupTableViewBackgroundColor];
        [_toolBar addSubview:lineso];
        
        //设置输入框
        UIView *vc  = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 20, 20)];
        vc.backgroundColor = [UIColor clearColor];
        _textField = [[HPGrowingTextView alloc]initWithFrame:CGRectMake(10,8, SCREEN_WIDTH - 68, 34)];
        _textField.layer.masksToBounds = YES;
        _textField.layer.cornerRadius = 17;
        _textField.font = RegularFont(16);
        _textField.placeholder = KLanguage(@"说点什么...");
        _textField.textColor = [UIColor colorWithRed:150/255.f green:150/255.f blue:150/255.f alpha:1.f];
        _textField.placeholderColor = [UIColor colorWithRed:150/255.f green:150/255.f blue:150/255.f alpha:1.f];
        ...
        finish.frame = CGRectMake(SCREEN_WIDTH - 44,8,34,34);
        ...
        tableheader = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT*0.3 , SCREEN_WIDTH,50)];
        tableheader.layer.masksToBounds = YES;
        tableheader.layer.cornerRadius = 10;
        tableheader.backgroundColor = [UIColor whiteColor];
        
        //显示评论的数量
        _allCommentLabels = [[UILabel alloc]initWithFrame:CGRectMake(20,0,SCREEN_WIDTH/2,50)];
        _allCommentLabels.textColor = ColorAlpha(@"#333333", 1);
        _allCommentLabels.text = [NSString stringWithFormat:@"%d %@",allcomments,KLanguage(@"评论")];
        ...
        UILabel *liness = [[UILabel alloc]initWithFrame:CGRectMake(0,49,SCREEN_WIDTH,1)];
        liness.backgroundColor = ColorAlpha(@"#f4f5f6", 1);//[UIColor groupTableViewBackgroundColor];
        [tableheader addSubview:liness];
        
        CGRect tabFrame ;
        if ([from isEqual:@"消息事件"]) {
            tabFrame = CGRectMake(0, 20+kStatusBarHeight, SCREEN_WIDTH, SCREEN_HEIGHT-KSafeHeight-20-kStatusBarHeight);
        }else {
            tabFrame = CGRectMake(0, SCREEN_HEIGHT*0.3, SCREEN_WIDTH, SCREEN_HEIGHT*0.7 - 50-KSafeHeight);
        }
        _tableview = [[UITableView alloc]initWithFrame:tabFrame];
        _tableview.delegate   = self;
        _tableview.dataSource = self;
        _tableview.separatorStyle = UITableViewCellSeparatorStyleNone;
        _tableview.backgroundColor = ColorAlpha(@"#f4f5f6",1);//RGB(248, 248, 248);
        ...
        UIView *spaceView = [[UIView alloc]initWithFrame:CGRectMake(0, _tableview.bottom-12, SCREEN_WIDTH, 15)];
        spaceView.backgroundColor = _tableview.backgroundColor;
        [self addSubview:spaceView];
        [self addSubview:_tableview];
        [self addSubview:_toolBar];
        nothingLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, _tableview.height/2-10, SCREEN_WIDTH, 20)];
        nothingLabel.font = [UIFont systemFontOfSize:13];
        nothingLabel.text = KLanguage(@"暂无评论，快来抢沙发吧") ;
        nothingLabel.textColor = ColorAlpha(@"#969696", 1);
        nothingLabel.textAlignment = NSTextAlignmentCenter;
        nothingLabel.hidden = YES;
        [_tableview addSubview:nothingLabel];
        //tableview顶部横线 和 顶部 view分割开
        tableviewLine = [[UILabel alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT*0.3 + 49,SCREEN_WIDTH,1)];
        tableviewLine.backgroundColor = [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1];//[UIColor colorWithRed:198/255.0 green:198/255.0 blue:198/255.0 alpha:1];
...
//        [PublicView showCommenting:_tableview];
        ...
        [footer setTitle:KLanguage(@"没有更多了哦~") forState:MJRefreshStateNoMoreData];
...
-(void)showActionSheet:(NSIndexPath*)indexPath model:(commentModel *)cModel{
    ...
    if ([_hostid isEqual:[LCUserInfoManager shareManager].userInfo.ID] || [minstr([userinfo valueForKey:@"id"]) isEqual:[LCUserInfoManager shareManager].userInfo.ID] ||[minstr([subdic valueForKey:@"touid"]) isEqual:[LCUserInfoManager shareManager].userInfo.ID]) {
...
-(void)delComments:(NSDictionary *)subdic index:(NSIndexPath *)indexPath{
    ...
    NSString *url = @"Video.delComments";
    ...
    [[LCNetWorkManager manager] requestUrl:url method:@"POST" params:dic success:^(id  _Nullable result) {
...
-(void)atFrends {
    ...
    if (!finish.selected) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _toolBar.frame = CGRectMake(0, _emojiV.top - 50, SCREEN_WIDTH, 50);
...
//    if ([state isEqual:@"关闭"]) {
//        [UIView animateWithDuration:0.3 animations:^{
//            _selV.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
...
- (void)growingTextViewDidChange:(HPGrowingTextView *)growingTextView {
    ...
    if (newText.length < 1) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:textView.text              attributes:@{NSForegroundColorAttributeName:[UIColor colorWithRed:150/255.f green:150/255.f blue:150/255.f alpha:1.f],NSFontAttributeName:[UIFont systemFontOfSize:16]}];
        
        NSArray *matches = [self findAllAt];
        
        for (NSTextCheckingResult *match in matches) {
            [string addAttribute:NSForegroundColorAttributeName value:ColorAlpha(@"#f2e50b", 1) range:NSMakeRange(match.range.location, match.range.length - 1)];
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_touid isEqual:[LCUserInfoManager shareManager].userInfo.ID]) {
    ...
    [_textField becomeFirstResponder];
    finish.selected = NO;
    NSString *path = [NSString stringWithFormat:@"%@:%@",KLanguage(@"回复给"),[userinfo valueForKey:@"user_nicename"]];
...
-(void)getNewCount:(int)counts{
     _allCommentLabels.text = [NSString stringWithFormat:@"%d %@",counts,KLanguage(@"评论")];
}
...
//        [PublicObj warnLogin];
    ...
    if (_textField.text.length == 0 || _textField.text == NULL || _textField.text == nil || [_textField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length == 0) {
        [SVProgressHUD showMaskViewWithFailure:KLanguage(@"请添加内容后再尝试")];
        return;
    }
    ...
    NSString *url = [NSString stringWithFormat:@"Video.setComment&videoid=%@&content=%@&uid=%@&token=%@&touid=%@&commentid=%@&parentid=%@&at_info=%@",_videoid,path,[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,sendtouid,sendcommentid,sendparentid,at_json];
...
//            [SVProgressHUD showMaskViewWithSuccess:@"评论成功"];
        [SVProgressHUD showMaskViewWithFailure:KLanguage(@"评论成功")];
...
- (void)keyboardWillShow:(NSNotification *)aNotification {
    ...
    _toolBar.frame = CGRectMake(0, height - 50, SCREEN_WIDTH, 50);
    //消息事件 tableview 全屏不处理frame
    if (![_fromWhere isEqual:@"消息事件"]) {
        self.tableview.frame = CGRectMake(0, SCREEN_HEIGHT*0.3 + 50 - yyyy/2, SCREEN_WIDTH, SCREEN_HEIGHT*0.7 - 84-kStatusBarHeight);
    }
...
- (void)keyboardWillHide:(NSNotification *)aNotification {
    ...
    [UIView animateWithDuration:0.1 animations:^{
       weakSelf.toolBar.frame = CGRectMake(0, SCREEN_HEIGHT - 50-kStatusBarHeight, SCREEN_WIDTH, 50+kStatusBarHeight);
        _emojiV.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, EmojiHeight+KSafeHeight);
     ...
     //消息事件 tableview 全屏不处理frame
    if (![_fromWhere isEqual:@"消息事件"]) {
        self.tableview.frame = CGRectMake(0, SCREEN_HEIGHT*0.3 + 50, SCREEN_WIDTH, SCREEN_HEIGHT*0.7 - 84-kStatusBarHeight);
    }
