...
-(void)setmyframe:(commentModel *)model{
    ...
    CGSize size = [str boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 100, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil].size;
    
    NSString *str2 = [NSString stringWithFormat:@"%@ %@",_replyContent,_replyDate];
    CGSize size2 = [str2 boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 120, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size;
    ...
    if (replys >1) {
        _ReplyRect = CGRectMake(50, _ReplyFirstRect.origin.y + _ReplyFirstRect.size.height + 5, SCREEN_WIDTH - 100,20);
