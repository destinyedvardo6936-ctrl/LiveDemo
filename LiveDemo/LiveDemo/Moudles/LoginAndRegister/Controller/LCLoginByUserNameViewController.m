//
//  LCLoginByUserNameViewController.m
//  LiveDemo
//
...
@interface LCLoginByUserNameViewController ()
...
@property (nonatomic , strong) LCLoginUserNameViewModel *viewModel;
@end

@implementation LCLoginByUserNameViewController
...
- (void)lc_addSubviews{
    ...
    [self.loginBgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(kUI_Width(308)+ (kUI_Width(42)+kUI_Width(10)));
    }];
...
//    }];
    ...
    [self.loginBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.registerBtn.mas_top).offset(-kUI_Width(10));
...
- (void)lc_bindViewModel{
    ...
    [[[RACSignal combineLatest:@[RACObserve(self.viewModel, account),RACObserve(self.viewModel, passward),RACObserve(self.viewModel, country_code)]] takeUntil:self.rac_willDeallocSignal] subscribeNext:^(RACTuple * _Nullable x) {
        @strongify(self);
        
            self.loginBtn.enabled = (self.viewModel.account.length > 0) && (self.viewModel.passward.length >0) && (self.viewModel.country_code.length > 0) ;
...
- (UIButton *)registerBtn{
    if(!_registerBtn){
        ...
        WS(weakSelf)
        [[btn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            LCRegisterByAccountViewController *con = [LCRegisterByAccountViewController new];
            [weakSelf pushToViewController:con];
        }];
    }
    return _registerBtn;
}
- (LCLoginUserNameViewModel *)viewModel{
    if(!_viewModel){
        _viewModel = [LCLoginUserNameViewModel new];
    }
    return _viewModel;
}
/*
