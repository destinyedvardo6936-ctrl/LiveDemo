...
@implementation LCActivitySegmentViewModel
- (void)lc_initialize{
...
//        [weakSelf.channelDataArray addObjectsFromArray:[LCGameConfigManager shareManager].gameStatus ?@[KLanguage(@"游戏活动"),KLanguage(@"直播活动")]:@[KLanguage(@"直播活动")]];
//    }];
...
//        [_channelDataArray addObjectsFromArray:[LCGameConfigManager shareManager].gameStatus?  @[KLanguage(@"游戏活动"),KLanguage(@"直播活动")] :@[KLanguage(@"直播活动")]];
    }
        ...
        _changeChannelIndexCommend = [[RACCommand alloc]initWithSignalBlock:^RACSignal * _Nonnull(NSNumber  *_Nullable input) {
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                if (input.integerValue < self.channelDataArray.count && input.integerValue >= 0) {
                    if(self.currentIndex != input.integerValue){
