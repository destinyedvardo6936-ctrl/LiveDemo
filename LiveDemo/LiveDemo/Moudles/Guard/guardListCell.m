...
- (void)setModel:(guardListModel *)model{
    ...
    _yingpiaoL.text = [LCConfigManager shareManager].configModel.name_votes;
    ...
    for (LCUserLevelModel *le in arr) {
        if([le.levelid isEqualToString:_model.level]){
            levelModel = le;
        }
    }
    
    [_levelImgView sd_setImageWithURL:[NSURL URLWithString:levelModel.thumb]];
