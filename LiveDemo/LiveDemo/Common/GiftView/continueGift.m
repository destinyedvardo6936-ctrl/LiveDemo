...
//    NSString *giftcount = @"1";
    ...
    NSString *content   = [NSString stringWithFormat:@"%@ %@",KLanguage(@"送"),giftname];
        ...
        if (_popListItem1!=0) {
            if (_popListItem1 == tagID && _previousGiftID1 == [giftData valueForKey:@"giftid"]) {
                ...
                if ([minstr([giftData valueForKey:@"isluck"]) isEqual:@"1"]) {
                    [self changePrizeBtnTitle:minstr([giftData valueForKey:@"lucktimes"])];
                }
                flag = 1;
            }
    ...
    labName.font = fontMT(15);
 
    labName.textColor = ColorAlpha(minstr([levelDic valueForKey:@"colour"]), 1);
    ...
    [contentStr addAttributes:@{NSForegroundColorAttributeName:normalColors,NSFontAttributeName:[UIFont systemFontOfSize:14]} range:redRange];
...
//        make.top.equalTo(giftBGView).offset(-15);
    ...
    giftXL.outLinetextColor = ColorAlpha(@"#bd6d26", 1);
    giftXL.outLineWidth  = 1.5;
    giftXL.font = [UIFont boldSystemFontOfSize:15];
    giftXL.labelTextColor = normalColors;
...
//    }];
    ...
    //礼物数量
    labGiftNum.outLinetextColor = ColorAlpha(@"#bd6d26", 1);
...
//    labGiftNum.colors = @[(id)ColorAlpha(@"#ffa800", 1).CGColor ,(id)ColorAlpha(@"#ffe400", 1).CGColor];
...
//    }];
    ...
    if (isLucky) {
        ...
        prizeBtn = [UIButton buttonWithType:0];
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (isLucky) {
            leftImgV.left = giftBGView.width-50;
            rightImgV.left = 0;
            leftImgV.alpha = 0;
            rightImgV.alpha = 0;
        }
...
- (void)changePrizeBtnTitle:(NSString *)title{
    prizeBtn.hidden = YES;
    [prizeBtn setTitle:[NSString stringWithFormat:@"%@%@",KLanguage(@"喜中"),title] forState:0];
    prizeBtn.left = -14;
    prizeBtn.hidden = NO;
