...
- (void)creatUIWithGiftData:(NSDictionary *)giftData andVideoitem:(SVGAVideoEntity * _Nullable)videoitem andIsPlat:(BOOL)ispalat{
    if (videoitem) {
        SVGAPlayer *player = [[SVGAPlayer alloc] initWithFrame:CGRectMake(0, (SCREEN_HEIGHT-(SCREEN_WIDTH/videoitem.videoSize.width*videoitem.videoSize.height))/2, SCREEN_WIDTH, (SCREEN_WIDTH/videoitem.videoSize.width*videoitem.videoSize.height))];
    ...
    if (ispalat || [minstr([giftData valueForKey:@"_method_"]) isEqual:@"Sendplatgift"]) {
        ...
        NSString *titleStr = [NSString stringWithFormat:@"%@ %@%@%@ %@",minstr([giftData valueForKey:@"uname"]),KLanguage(@"在"),minstr([giftData valueForKey:@"livename"]),KLanguage(@"的直播间送出"),minstr([giftData valueForKey:@"giftname"])];
        CGFloat titleWidth = [titleStr sizeForFont:[UIFont systemFontOfSize:14] size:CGSizeMake(CGFLOAT_MAX, 30) mode:NSLineBreakByCharWrapping].width;
        UIImageView *titleBackImgView = [[UIImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH, 110, 35+titleWidth+20, 30)];
    //    titleBackImgView.image = [UIImage imageNamed:@"moviePlay_title"];
        titleBackImgView.image = [UIImage imageNamed:@"allPlatFormGiftBGNew"];
        ...
        laba.image = [UIImage imageNamed:@"qzlbGiftImg"];
        ...
        NSString *titleStr = [NSString stringWithFormat:@"%@ %@%@",minstr([giftData valueForKey:@"nicename"]),KLanguage(@"送了一个"),minstr([giftData valueForKey:@"giftname"])];
        CGFloat titleWidth = [titleStr sizeForFont:[UIFont systemFontOfSize:14] size:CGSizeMake(CGFLOAT_MAX, 30) mode:NSLineBreakByCharWrapping].width;
        UIImageView *titleBackImgView = [[UIImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH, 110, 35+titleWidth+20, 30)];
        titleBackImgView.image = [UIImage imageNamed:@"moviePlay_title"];
        titleBackImgView.alpha = 0.5;
        ...
        [self addSubview:titleBackImgView];
        UIImageView *laba = [[UIImageView alloc]initWithFrame:CGRectMake(10, 7.5, 15, 15)];
        laba.image = [UIImage imageNamed:@"expensiveGiftTip"];
        [titleBackImgView addSubview:laba];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(seconds*0.75 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:seconds/4 animations:^{
                ...
                titleBackImgView.left = -SCREEN_WIDTH;
            } completion:^(BOOL finished) {
