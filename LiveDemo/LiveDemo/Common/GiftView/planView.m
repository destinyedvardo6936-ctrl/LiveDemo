...
#define planW  SCREEN_WIDTH
#define planH  SCREEN_HEIGHT/2
@interface planView ()
{
...
-(instancetype)initWithicon:(NSString *)icons andName:(NSString *)name{
    ...
    if (self) {
        ...
        buttomView = [[UIView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH,0,planW, planH)];
      ...
      timerLabel =  [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(random) userInfo:nil repeats:YES];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.7 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:2.0 animations:^{
                buttomView.frame=CGRectMake(-SCREEN_WIDTH,0,planW,planH);
             }];
...
-(void)random{
    ...
    CGFloat randomX = (CGFloat)(arc4random()% 161)+SCREEN_WIDTH;
    CGFloat randomY = (CGFloat)(arc4random()% 161)+SCREEN_HEIGHT;
