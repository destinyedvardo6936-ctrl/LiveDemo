...
- (void)viewDidLoad {// add model
  [super viewDidLoad];
    ...
    self.view.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
    self.albumTable.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
    self.albumTable.separatorStyle = UITableViewCellSeparatorStyleNone;
//  [self setupNavigationBar];
    self.tabelTop.constant = kNavBarHeight;
    [self creatNavi];
...
- (void)pushToSelectPhotoVCWithIndex:(NSInteger)index {
  ...
  selectPhotoVC.photoDelegate = _photoDelegate;
    [[MXBADelegate sharedAppDelegate]pushViewController:selectPhotoVC animated:YES];
