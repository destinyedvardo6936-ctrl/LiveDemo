...
- (void)viewDidLoad {
  ...
  self.navigationController.navigationBar.translucent = NO;
    ...
    bottomV.backgroundColor = [UIColor whiteColor];// ColorAlpha(@"#2c2840", 1);
  selectedPhotoDic = @{}.mutableCopy;
  allPhotoArr = @[].mutableCopy;
//  self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:KLanguage(@"取消")
...
- (void)getAllPhotoWithAssert {
        [[[ALAssetsLibrary alloc] init] enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
          ...
          UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(KLanguage(@"提示"),nil) message:NSLocalizedString(@"请允许访问相册,方可使用此功能!\n您可以在\"设置->隐私->照片\"中启用",nil) delegate:self cancelButtonTitle:NSLocalizedString(KLanguage(@"取消"),nil) otherButtonTitles:NSLocalizedString(@"设置",nil), nil];
          [alertView show];
        }];
}
- (void)setUpCollectionView {
    self.collectionTop.constant = kNavBarHeight;
    self.botToolHeigh.constant = 45+KSafeHeight;
  ...
  UICollectionViewFlowLayout *collectLayout = (UICollectionViewFlowLayout *)photoGridView.collectionViewLayout;
    collectLayout.itemSize = CGSizeMake(SCREEN_WIDTH/4-1, SCREEN_WIDTH/4-1);
  ...
  if ([UINavigationBar appearance].translucent == YES) {
    ...
    photoGridView.backgroundColor = ColorAlpha(@"#f4f5f6",1);
...
- (IBAction)clickToBrowserSelectedPhotos:(id)sender {
  ...
  photoBrowserVC.currentIndex = browserIndex;
    [[MXBADelegate sharedAppDelegate]pushViewController:photoBrowserVC animated:YES];
...
- (void)didSelectStatusChange:(JCHATPhotoModel *)model {
    ...
    if (selectedPhotoDic[model.photoAsset] == nil) {
      if (selectedPhotoDic.count > 8) {
        //[MBProgressHUD showMessage:@"最多选择9张图片" view:self.view];
          [SVProgressHUD showMaskViewWithFailure:@"最多选择9张图片"];
        return;
      }
      [selectedPhotoDic setObject:model forKey:model.photoAsset];
    } else {
  ...
  if ([selectedPhotoDic count] > 0) {
    browserBtn.enabled = YES;
      [browserBtn setTitleColor:[UIColor colorWithRed:255/255.0 green:88/255.0 blue:120/255.0 alpha:1] forState:0];
    selectedPhotoLabel.hidden = NO;
    selectedPhotoLabel.text = [NSString stringWithFormat:@"%ld",(unsigned long)[selectedPhotoDic count]];
    sendBtn.enabled = YES;
      [sendBtn setTitleColor:[UIColor colorWithRed:255/255.0 green:88/255.0 blue:120/255.0 alpha:1] forState:0];
  } else {
    browserBtn.enabled = NO;
      [browserBtn setTitleColor:ColorAlpha(@"#8c8c8c", 0.6) forState:0];
    selectedPhotoLabel.hidden = YES;
    sendBtn.enabled = NO;
      [sendBtn setTitleColor:ColorAlpha(@"#8c8c8c", 0.6) forState:0];
  }
}
...
-(void)creatNavi {
    ...
    } andRightName:KLanguage(@"取消") andRight:^(id btnBack) {
        [self dismissViewControllerAnimated:YES completion:nil];
    } andMidTitle:@"相机胶卷"];
