...
- (void)createScrollView
{
    ...
    UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(savePhoneLongTap)];
    [_photoScrollView addGestureRecognizer:longTap];
...
//    UIAlertAction *saveAction = [UIAlertAction actionWithTitle:KLanguage(@"保存到本地相册") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//
//    }];
//    UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:KLanguage(@"取消") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
//
//    }];
//    [sheetAlert addAction:saveAction];
//    [sheetAlert addAction:cancleAction];
...
////    }
//
//    [[[MXBADelegate sharedAppDelegate] topViewController]presentViewController:sheetAlert animated:YES completion:nil];
...
//        [SDWebImageManager downloadWithURL:photo.url];
        ...
        [[SDWebImageDownloader sharedDownloader]downloadImageWithURL:photo.url options:1 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
                    
        }];
        
//        [[SDWebImageManager sharedManager] downloadImageWithURL:photo.url options:1 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
//
//        }];
    }
