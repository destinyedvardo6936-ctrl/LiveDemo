...
- (void)setupTableView {
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0, SCREEN_WIDTH, self.height) style:UITableViewStylePlain];
    self.tableView.dataSource = self;
    self.tableView.delegate = self;
    self.tableView.tableFooterView = [[UIView alloc] init];
    self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.tableView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
    self.tableView.backgroundColor =[UIColor whiteColor];// ColorAlpha(@"#f4f5f6",1);
...
- (void)refreshData:(NSNotification *)notification {
    ...
    if (self.pageIndex == 1) {
     ...
     if (self.dataSource.count == 0) {
         [SVProgressHUD showMaskViewWithFailure:KLanguage(@"暂无数据")];
            
         }else{
//             [PublicView hiddenTextNoData:self.tableView];
         }
