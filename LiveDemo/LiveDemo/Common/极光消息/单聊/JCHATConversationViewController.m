...
@interface JCHATConversationViewController ()<UIGestureRecognizerDelegate,twEmojiViewDelegate,TouchMsgTabDelegate,TZImagePickerControllerDelegate> {
...
- (void)viewDidLoad {
    ...
    [self creatNavi];
    self.messageTabTop.constant = kNavBarHeight;
...
- (void)creatFollowView{
    followView = [[UIView alloc]initWithFrame:CGRectMake(0, kNavBarHeight, SCREEN_WIDTH, 40)];
    ...
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(fCloseBtn.right, 0, SCREEN_WIDTH/2, 40)];
    label.font = [UIFont systemFontOfSize:12];
    label.text = KLanguage(@"点击关注，可及时看到对方动态");
    label.textColor = ColorAlpha(@"#959697", 1);
    [followView addSubview:label];
    UIButton *followBtn = [UIButton buttonWithType:0];
    followBtn.frame = CGRectMake(SCREEN_WIDTH-54, 10, 44, 20);
...
- (void)doFollow{
    ...
    [YBToolClass postNetworkWithUrl:postUrl andParameter:postDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            [SVProgressHUD showMaskViewWithFailure:KLanguage(@"关注对方成功")];
            if (self.block) {
                self.block(1);
            }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch;{
    if ([touch.view isDescendantOfView:_asrView]||
        [touch.view isDescendantOfView:_emojiV]||
        [touch.view isDescendantOfView:_toolBarContainer]||
        [touch.view isDescendantOfView:_moreViewContainer]) {
        return NO;
    }
    return YES;
}
...
- (void)setupView {
  ...
  [self.view setBackgroundColor:[UIColor whiteColor]];
    _toolBarToBottomConstrait.constant = 0+KSafeHeight;
  ...
  self.toolBarContainer.toolbar.textView.text = [[JCHATSendMsgManager ins] draftStringWithConversation:_conversation];
    self.toolBarContainer.toolbar.textView.layer.cornerRadius = 18;
  ...
  _messageTableView.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
  
  _moreViewContainer.moreView.delegate = self;
  _moreViewContainer.moreView.backgroundColor = ColorAlpha(@"#f4f5f6",1);
    
    //表情
    _emojiV = [[twEmojiView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, EmojiHeight+KSafeHeight)];
...
#pragma mark - 消息撤回
//撤回消息-重置声音播放和图片放大
-(void)retractResetVoiceOrImg {
    if ([[JCHATAudioPlayerHelper shareInstance] isPlaying]) {
        [[JCHATAudioPlayerHelper shareInstance] stopAudio];
    }
    if ([[[MXBADelegate sharedAppDelegate] topViewController] isKindOfClass:[MJPhotoBrowser class]]) {
        MJPhotoBrowser *pBrowser = (MJPhotoBrowser *)[[MXBADelegate sharedAppDelegate] topViewController];
        [pBrowser retractMsgDismissBrowser];
    };
    
}
- (void)onReceiveMessageRetractEvent:(JMSGMessageRetractEvent *)retractEvent;{
    NSLog(@"----:%@-%@",retractEvent,retractEvent.retractMessage);
    [self retractResetVoiceOrImg];
    [self retractMsgReplace:retractEvent.retractMessage];
}
-(void)retractMsgReplace:(JMSGMessage *)jMessage {
    NSString *retractMsgId = jMessage.msgId;
    JCHATChatModel *model = [_allMessageDic objectForKey:retractMsgId];
    [model setChatModelWith:jMessage conversationType:_conversation userModel:_userModel];

    [_allMessageDic removeObjectForKey:retractMsgId];
    [_allMessageDic setObject:model forKey:model.message.msgId];

    //如果撤回的是图片图片数组要处理一下
    NSMutableArray *need_del_array = [NSMutableArray array];
    for (JCHATChatModel *imgModel in _imgDataArr) {
        if ([imgModel.message.msgId isEqual:model.message.msgId]) {
            [need_del_array addObject:imgModel];
        }
    }
    [_imgDataArr removeObjectsInArray:need_del_array];
...
#pragma mark
-(void)userRetractMessage:(NSNotification *)notification {
    JMSGMessage *message = notification.object;
    NSLog(@"撤回===%@",message.msgId);
    [_conversation retractMessage:message completionHandler:^(id resultObject, NSError *error) {
        NSLog(@"===ch:%@-%@",resultObject,error);
        NSLog(@"撤回===2%@",resultObject);
        if (error != nil) {
            //[SVProgressHUD showMaskViewWithFailure:KLanguage(@"该消息发送时间已超过两分钟,无法撤回")];
        }else {
            [self retractMsgReplace:(JMSGMessage *)resultObject];
        }
    }];
...
//    photoPickerVC.modalPresentationStyle = UIModalPresentationFullScreen;
//    [self presentViewController:photoPickerVC animated:YES completion:NULL];
//  } failureBlock:^(NSError *error) {
//
//      UIAlertController *alertC = [UIAlertController alertControllerWithTitle:KLanguage(@"没有相册权限") message:KLanguage(@"请到设置页面获取相册权限") preferredStyle:UIAlertControllerStyleAlert];
//      UIAlertAction *cancelA = [UIAlertAction actionWithTitle:KLanguage(@"确定") style:UIAlertActionStyleCancel handler:nil];
...
-(void)voiceInputClick {
    ...
    WS(weakSelf);
    if (!_asrView) {
        _asrView = [[ASRView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT) callBack:^(NSString *type, NSString *content) {
            if ([type isEqual:@"返回"]) {
                NSLog(@"返回");
                [weakSelf asrBack];
            }else if ([type isEqual:KLanguage(@"发送")]){
                NSLog(@"发送");
    ...
    [UIView animateWithDuration:0.25 animations:^{
        _asrView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
...
-(void)asrBack {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        _asrView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
        _moreViewHeight.constant = kMoreHeight+KSafeHeight;
        [self scrollToEnd];
    }];
}
...
-(void)locationClick {
    ...
    [[MXBADelegate sharedAppDelegate]pushViewController:locationVC animated:YES];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
  ...
  if ([mediaType isEqualToString:@"public.movie"]) {
    ...
    [MBProgressHUD showMessage:KLanguage(@"不支持视频发送") toView:self.view];
    return;
  }
  UIImage *image;
...
- (void)addNotification{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(AlertToSendImage:)
                                             ...
                                             object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                               selector:@selector(userRetractMessage:)
                                                   name:kRetractMsg
                                                 object:nil];
...
- (void)dropToolBar {
    if (_ybNavi.hidden) {
        [UIView animateWithDuration:0.3 animations:^{
            self.view.frame = CGRectMake(0, SCREEN_HEIGHT - SCREEN_HEIGHT*0.4, SCREEN_WIDTH, SCREEN_HEIGHT*0.4);
            _emojiV.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, EmojiHeight);
        }];
        return;
    }
...
//  [_messageTableView reloadData];
  [UIView animateWithDuration:0.3 animations:^{
    _toolBarToBottomConstrait.constant = 0+KSafeHeight;
      _emojiV.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, EmojiHeight+KSafeHeight);
    _moreViewHeight.constant = 0;
  }];
...
- (void)inputKeyboardWillShow:(NSNotification *)notification{
    ...
    CGFloat newHeight = SCREEN_HEIGHT - height;

    if (_ybNavi.hidden) {
        [UIView animateWithDuration:0.3 animations:^{
            self.view.frame = CGRectMake(0,SCREEN_HEIGHT*0.2, SCREEN_WIDTH, SCREEN_HEIGHT*0.8 - newHeight);
        }];
        //        mview.frame = CGRectMake(0,self.view.frame.size.height-barH, SCREEN_WIDTH*0.2-20, 44);
        return;
    }
    ...
    if (_asrView.textView.isFirstResponder == YES) {
        [UIView animateWithDuration:0.25 animations:^{
            _asrMoveUp = YES;
            _asrView.frame = CGRectMake(0, -keyBoardFrame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT);
...
-(void)pressFaceBtnClick:(UIButton *)btn {
    ...
    if (_ybNavi.hidden) {
        [UIView animateWithDuration:0.3 animations:^{
            self.view.frame = CGRectMake(0,SCREEN_HEIGHT*0.2, SCREEN_WIDTH, SCREEN_HEIGHT*0.8 - EmojiHeight);
            _emojiV.frame = CGRectMake(0, SCREEN_HEIGHT-EmojiHeight-KSafeHeight, SCREEN_WIDTH, EmojiHeight+KSafeHeight);
    ...
    _barBottomFlag=NO;
    [UIView animateWithDuration:0.3 animations:^{
        _toolBarToBottomConstrait.constant = EmojiHeight+KSafeHeight;
        _moreViewHeight.constant = 0;
        _emojiV.frame = CGRectMake(0, SCREEN_HEIGHT-EmojiHeight-KSafeHeight, SCREEN_WIDTH, EmojiHeight+KSafeHeight);
        [_messageTableView layoutIfNeeded];
        [_toolBarContainer layoutIfNeeded];
...
- (void)prepareTextMessage:(NSString *)text {
    ...
    //屏蔽词过滤
    NSArray *wordArr = [common sensitive_words];
    NSString *sendMsg =text;
    ...
    JMSGTextContent *textContent = [[JMSGTextContent alloc] initWithText:sendMsg];
...
//          [self performSelector:@selector(flashToLoadMessage) withObject:nil afterDelay:0];
    ...
    if (model.isTime == YES || model.message.contentType == kJMSGContentTypeEventNotification || model.isErrorMessage|| model.message.contentType == kJMSGContentTypePrompt) {
        //消息时间
        ...
        }else if (model.message.contentType == kJMSGContentTypePrompt) {
            NSString *alertMsg = KLanguage(@"你撤回了一条消息");
            if (model.message.isReceived) {
                alertMsg = KLanguage(@"对方撤回了一条消息");
            }
            cell.messageTimeLabel.text = alertMsg;
        } else {
        ...
        cell.passRKTextView = self.toolBarContainer.toolbar.textView;
...
//    if (![model.message isReceived]) {
//        //自己
//        center.otherUid =[LCUserInfoManager shareManager].userInfo.ID;
//    }else {
//        //别人
...
//  browser.currentPhotoIndex = cell.model.photoIndex; // 弹出相册时显示的第一张图片是？
  ...
  browser.saveEvent = ^(UIImage *selImg) {
        selSavaImg = selImg;
        UIWindow *window = [UIApplication sharedApplication].keyWindow;
        [window addSubview:self.actionSheetView];
        self.actionSheetView.hidden = NO;
//        [UIView animateWithDuration:0.5 animations:^{
//            self.actionSheetView.frame =CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
//        }];
    };
  [browser show];
}
-(UIView *)actionSheetView{
    if (!_actionSheetView) {
        _actionSheetView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
        _actionSheetView.backgroundColor = [UIColor clearColor];
        
        
        UIView *backView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT-150, SCREEN_WIDTH, 150)];
        backView.backgroundColor = [UIColor clearColor];
        [_actionSheetView addSubview:backView];
        
        UIButton *btn1 = [UIButton buttonWithType:0];
        btn1.frame= CGRectMake(10, 10, SCREEN_WIDTH-20, 55);
        [btn1 setBackgroundColor:[UIColor whiteColor]];
        [btn1 setTitle:KLanguage(@"保存到本地相册") forState:0];
        [btn1 setTitleColor:[UIColor blackColor] forState:0];
        btn1.titleLabel.font = [UIFont systemFontOfSize:14];
        btn1.layer.cornerRadius = 5;
        btn1.layer.masksToBounds = YES;
        [btn1 addTarget:self action:@selector(saveImgClick) forControlEvents:UIControlEventTouchUpInside];
        [backView addSubview:btn1];
        
        UIButton *btn2 = [UIButton buttonWithType:0];
        btn2.frame= CGRectMake(10, btn1.bottom+10, SCREEN_WIDTH-20, 55);
        [btn2 setBackgroundColor:[UIColor whiteColor]];
        [btn2 setTitle:KLanguage(@"取消") forState:0];
        [btn2 addTarget:self action:@selector(cancleClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    _actionSheetView.hidden = YES;
    return _actionSheetView;
}
-(void)cancleClick{
//    [UIView animateWithDuration:2 animations:^{
//        self.actionSheetView.frame =CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
//
...
-(void)saveImgClick{
    UIImageWriteToSavedPhotosAlbum(selSavaImg, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
...
-(void)checkBlack:(JMSGAbstractContent *)content voice:(NSString *)voicePath {
    ...
    NSString *url = [purl stringByAppendingFormat:@"/?service=User.checkBlack&uid=%@&token=%@&touid=%@",[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,_userModel.uidStr];
...
//                NSString *msgid = [_allmessageIdArr objectAtIndex:i];
//                JCHATChatModel *model = [_allMessageDic objectForKey:msgid];
//                if ([model.uidStr isEqual:[LCUserInfoManager shareManager].userInfo.ID]) {
...
//                    [SVProgressHUD showMaskViewWithFailure:[NSString stringWithFormat:@"%@%d%@",KLanguage(@"对方未关注你,最多只能发送"),p_nums,KLanguage(@"条信息")]];
//                }else {
                    JMSGMessage *message = nil;
...
//                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"对方暂时拒绝接收您的消息")];
                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"对方暂时拒绝接收您的消息")];
            }
        }else {
//            [SVProgressHUD showMaskViewWithFailure:msg];
...
#pragma mark - 拉黑
-(void)setBlack {
    NSString *url = [purl stringByAppendingFormat:@"/?service=User.setBlack&uid=%@&token=%@&touid=%@",[LCUserInfoManager shareManager].userInfo.ID,[LCUserInfoManager shareManager].userInfo.token,_userModel.uidStr];
    [YBNetworking postWithUrl:url Dic:nil Suc:^(NSDictionary *data, NSString *code, NSString *msg) {
        if ([code isEqual:@"0"]) {
...
//                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"拉黑成功")];
                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"拉黑成功")];
                
            }else {
//                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"取消拉黑成功")];
                [SVProgressHUD showMaskViewWithFailure:KLanguage(@"取消拉黑成功")];
            }
        }else {
//            [SVProgressHUD showMaskViewWithFailure:msg];
...
-(void)creatNavi {
    ...
    [_ybNavi ybNaviLeft:^(id btnBack) {
        ...
        if ([self.fromWhere isEqual:@"otherUserMsgVC"]) {
            [[MXBADelegate sharedAppDelegate]popViewController:YES];
        }else{
            [self clickRightMore];
        }
    } andMidTitle:_userModel.unameStr];
    [[YBToolClass sharedInstance] lineViewWithFrame:CGRectMake(0, _ybNavi.height-1, SCREEN_WIDTH, 1) andColor:RGB(244, 245, 246) andView:_ybNavi];
...
- (void)reloadSamllChtaView:(NSString *)isatt{
    _ybNavi.hidden = YES;
    UIView *smallNavi = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 35)];
    smallNavi.backgroundColor = ColorAlpha(@"#f9fafb", 1);
    ...
    UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(35, 0, SCREEN_WIDTH-70, 35)];
    titleLabel.font = [UIFont systemFontOfSize:13];
    titleLabel.textColor= ColorAlpha(@"#636464", 1);
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.text = _userModel.unameStr;
    [smallNavi addSubview:titleLabel];
    self.messageTabTop.constant = 35;
    _messageTableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT*0.4-80);
    [self getGroupMemberListWithGetMessageFlag:YES];
