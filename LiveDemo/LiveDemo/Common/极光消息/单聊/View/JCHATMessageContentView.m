...
- (void)setMessageContentWith:(JMSGMessage *)message handler:(void (^)(NSUInteger))block {
    ...
    if (isReceived) {
        ...
        _textContent.textColor = ColorAlpha(@"#333333", 1);

    } else {
//        maskImage = [UIImage sd_imageWithData:<#(nullable NSData *)#>]
...
//    self.layer.mask = maskView.layer;
    ...
    switch (message.contentType) {
        ...
        case kJMSGContentTypeLocation: {
            ...
            _textContent.textColor  = ColorAlpha(@"#323232", 1);
            _infoL.textColor = ColorAlpha(@"#646464", 1);
            
            if (isReceived) {
                self.backgroundColor = ColorAlpha(@"#ffffff", 1);
...
//
//            if (isReceived) {
//                self.backgroundColor = ColorAlpha(@"#ffffff", 1);//RGB(44, 42, 62);
//
//                [_textContent setFrame:CGRectMake(textMessageContentRightOffset + 5, textMessageContentTopOffset, self.frame.size.width - 2 * textMessageContentRightOffset, 20)];
//                [_infoL setFrame:CGRectMake(textMessageContentRightOffset + 5, _textContent.bottom, self.frame.size.width - 2 * textMessageContentRightOffset, 15)];
//            } else {
...
-(void)handleTap:(UIGestureRecognizer*) recognizer {
    ...
    UIMenuItem *copyItem = [[UIMenuItem alloc]initWithTitle:KLanguage(@"复制") action:@selector(clickCopyItem)];
    UIMenuItem *retractItem = [[UIMenuItem alloc]initWithTitle:KLanguage(@"撤回") action:@selector(clickRetractItem)];
