...
- (id)initWithStyle:(UITableViewCellStyle)style
    ...
    if (self) {
        ...
        _voiceTimeLabel.textColor = ColorAlpha(@"#646464", 1);
...
- (void)setPassRKTextView:(JCHATMessageTextView *)passRKTextView {
    _passRKTextView = passRKTextView;
    _messageContent.passRKTextView = _passRKTextView;
}
...
- (void)setCellData:(JCHATChatModel *)model
    ...
    //业务服务器头像
    [_headView sd_setImageWithURL:[NSURL URLWithString:_model.uiconStr]];
...
- (void)reSendMessage {//重发消息
    ...
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:nil message:@"是否重新发送消息" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelA = [UIAlertAction actionWithTitle:KLanguage(@"取消") style:UIAlertActionStyleCancel handler:nil];
    UIAlertAction *sureA = [UIAlertAction actionWithTitle:KLanguage(@"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self.sendFailView setHidden:YES];
...
- (void)changeVoiceImage {
    ...
    _messageContent.voiceConent.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@%zd", voiceImagePreStr, self.index % 3]];
    if (_isPlaying) {
        self.index++;
...
#pragma mark - 极光IM存储的头像(业务服务器未使用)
