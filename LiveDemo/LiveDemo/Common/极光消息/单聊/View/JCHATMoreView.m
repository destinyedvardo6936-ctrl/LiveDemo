...
- (void)awakeFromNib {
  [super awakeFromNib];
    self.frame = CGRectMake(0, 0, SCREEN_WIDTH, kMoreHeight);
    self.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
  _moreView = NIB(JCHATMoreView);
  
  _moreView.frame =CGRectMake(0, 0, SCREEN_WIDTH, kMoreHeight);
  _moreView.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
    _moreView.tupianL.text = KLanguage(@"相册");
    _moreView.weizhiL.text = KLanguage(@"位置");
    _moreView.paisheL.text = KLanguage(@"拍摄");
    _moreView.yuyinL.text = KLanguage(@"语音输入");
