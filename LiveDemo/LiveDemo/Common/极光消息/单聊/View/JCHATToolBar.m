...
- (void)switchInputMode {
  if (self.voiceButton.selected == NO) {
      ...
      if (isShowLive) {
          [SVProgressHUD showMaskViewWithFailure:KLanguage(@"直播间开启期间不可使用该操作")];
          return;
...
- (void)drawRect:(CGRect)rect {
  ...
  self.startRecordButton = [UIButton new];
    ...
    [self.startRecordButton setTitleColor:ColorAlpha(@"#969696", 1) forState:UIControlStateNormal];
  [self.startRecordButton setTitleColor: ColorAlpha(@"#969696", 1) forState:UIControlStateHighlighted];
  [self.startRecordButton setTitle:KLanguage(@"按住 说话") forState:UIControlStateNormal];
  [self.startRecordButton setTitle:KLanguage(@"松开 结束") forState:UIControlStateHighlighted];
    self.startRecordButton.layer.masksToBounds = YES;
    self.startRecordButton.layer.cornerRadius = 17;
  self.startRecordButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin;
  [self.startRecordButton setBackgroundColor:ColorAlpha(@"#f4f5f6", 1)];
...
- (void)holdDownButtonTouchDown {
    //[self.startRecordButton setBackgroundColor:ColorAlpha(@"#27233a", 1)];
...
- (void)holdDownButtonTouchUpOutside {
    //[self.startRecordButton setBackgroundColor:ColorAlpha(@"#2c2840", 1)];
      ...
      [audioSession performSelector:@selector(requestRecordPermission:) withObject:^(BOOL granted) {
        if (granted) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
              UIAlertController *alertC = [UIAlertController alertControllerWithTitle:KLanguage(@"无法录音") message:KLanguage(@"请在“设置-隐私-麦克风”选项中，允许jpushIM访问你的手机麦克风。") preferredStyle:UIAlertControllerStyleAlert];
              UIAlertAction *cancelA = [UIAlertAction actionWithTitle:KLanguage(@"关闭") style:UIAlertActionStyleCancel handler:nil];
...
- (void)awakeFromNib {
  ...
  _toolbar = NIB(JCHATToolBar);
    _toolbar.backgroundColor = ColorAlpha(@"#ffffff", 1);
    _toolbar.textView.backgroundColor = ColorAlpha(@"#f4f5f6", 1);
    _toolbar.textView.placeHolder = KLanguage(@"发送消息...");
    _toolbar.textView.placeHolderTextColor = ColorAlpha(@"#969696", 1);
    _toolbar.textView.textColor = ColorAlpha(@"#969696", 1);
