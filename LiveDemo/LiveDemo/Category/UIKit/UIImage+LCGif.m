...
+ (UIImage *)GIFName:(NSString *)name scale:(NSUInteger)scale
{
   ...
   NSString *imagePath = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@", name] ofType:@"gif"];
...
+ (void)imageWithGIFUrl:(NSString *)url and:(GIFimageBlock)gifImageBlock
{
 ...
 /** 根据一个GIF图片的URL 获得 image数组对象 */
+ (NSArray *)imageWithWithGIFName:(NSString *)name{
    NSURL *fileUrl = [[NSBundle mainBundle] URLForResource:name withExtension:@"gif"];
    NSData *CIFData = [NSData dataWithContentsOfURL:fileUrl];
    CGImageSourceRef source = CGImageSourceCreateWithData((__bridge CFDataRef)CIFData, NULL);
    
    size_t count = CGImageSourceGetCount(source);
    NSMutableArray* frames=[[NSMutableArray alloc] init];//定义数组存储拆分出来的图片
      for (size_t i=0; i<count;i++){

        CGImageRef imageRef=CGImageSourceCreateImageAtIndex(source, i, NULL);//从GIF图片中取出源图片
