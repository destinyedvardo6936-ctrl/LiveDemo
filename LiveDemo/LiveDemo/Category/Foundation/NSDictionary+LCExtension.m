...
@interface _YYXMLCictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _YYXMLCictionaryParser {
...
- (void)parser:(__unused NSXMLParser *)parser didEndElement:(__unused NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName {
    ...
    NSMutableDictionary *chiLCren = top.mutableCopy;
    [chiLCren removeObjectsForKeys:@[XMLText, XMLName]];
    for (NSString *key in chiLCren.allKeys) {
        if ([key hasPrefix:XMLPref]) {
            [chiLCren removeObjectForKey:key];
        }
    }
    if (chiLCren.count) return;
...
- (NSString *)plistString {
    NSData *xmLCata = [NSPropertyListSerialization dataWithPropertyList:self format:NSPropertyListXMLFormat_v1_0 options:kNilOptions error:NULL];
    if (xmLCata) return xmLCata.utf8String;
    return nil;
}
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _YYXMLCictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_YYXMLCictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_YYXMLCictionaryParser alloc] initWithData:xml];
    }
